...
**Sprint 3.5**: `CupCanvasLoader`/`CupCanvas`/`CupScene` gained an optional `cameraPreset?: CameraPresetName` prop, defaulting to `"hero"` — every prior caller keeps its exact prior camera. `CupCanvas` now resolves its *initial* `<Canvas camera={...}>` seed from this prop instead of a hardcoded module-level `heroCameraPreset` constant. `features/concierge/` is the first real caller with a non-default value (`"ai"`, a new registered preset — see `engine/camera/presets.ts`), making `CameraRig`'s preset-to-preset smooth interpolation (built Sprint 2.1, previously only unit-tested, never exercised by a live second preset) real for the first time.

**Sprint 3.8 (Final Polish)**: two real bugs found by a dedicated audit and fixed. (1) `ProceduralCoffee`/`ProceduralFoam`'s `materialOverrides` branch bypassed the shared material cache entirely — every override change re-ran a full shader recompile instead of hitting an already-compiled cache entry, the same landmine every other part (`Cup`/`Sleeve`/`Lid`) had already been fixed against in Sprint 3.2. Now routed through `getOrCreateMaterial`/`materialOverridesToVariant` identically to those parts. (2) `CupScene.tsx`/`CupAssembly.tsx` now publish real values into 5 of `engine/shaders/common/uniforms.ts`'s shared uniform block (`uThemeMode`/`uColorTheme`, `uQualityTier`, `uLightingIntensity`, `uIsInteracting`) — typed since Sprint 2.1/2.5 with real publisher functions that had zero call sites anywhere, silently frozen at their module-load defaults. `useLiquidPhysics.ts` gained a 6th, `uPhysicsIntensity` (typed since Sprint 2.4, "Milestone 3," never wired even after Sprint 3.4's physics work shipped) — now published from the same `qualityPolicy.coffeePhysics.intensity` scale that hook already uses for its own tilt/ripple amplitude.

## Flow

1. `app/page.tsx` renders `Hero` (Server Component); `app/customize/page.tsx` renders `features/customizer/`'s `CustomizerExperience`, which uses `CupCanvasLoader` the same way, with real override props. `app/concierge/page.tsx` (Sprint 3.5) does the same, additionally passing `cameraPreset="ai"` once a recommendation exists.
