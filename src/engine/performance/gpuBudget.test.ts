...
describe("checkGpuBudget", () => {
  ...
  it("does not warn when within budget", () => {
    ...
    checkGpuBudget(createPerformanceSnapshot({ fps: 60, drawCalls: 10, triangles: 1000, geometries: 5, gpuTextures: 2 }));
  ...
  it("warns once when draw calls exceed the budget", () => {
    const listener = vi.fn();
    const unsub = appEvents.on("gpu:budget-warning", listener);
    checkGpuBudget(createPerformanceSnapshot({ fps: 60, drawCalls: 150, triangles: 1000, geometries: 5, gpuTextures: 2 }));
  ...
  it("does not re-warn on every sample while draw calls stay over budget", () => {
    ...
    const unsub = appEvents.on("gpu:budget-warning", listener);
    checkGpuBudget(createPerformanceSnapshot({ fps: 60, drawCalls: 150, triangles: 1000, geometries: 5, gpuTextures: 2 }));
    checkGpuBudget(createPerformanceSnapshot({ fps: 60, drawCalls: 160, triangles: 1000, geometries: 5, gpuTextures: 2 }));
  ...
  it("warns again after dropping back under budget and crossing over a second time", () => {
    ...
    const unsub = appEvents.on("gpu:budget-warning", listener);
    checkGpuBudget(createPerformanceSnapshot({ fps: 60, drawCalls: 150, triangles: 1000, geometries: 5, gpuTextures: 2 }));
    checkGpuBudget(createPerformanceSnapshot({ fps: 60, drawCalls: 50, triangles: 1000, geometries: 5, gpuTextures: 2 }));
    checkGpuBudget(createPerformanceSnapshot({ fps: 60, drawCalls: 150, triangles: 1000, geometries: 5, gpuTextures: 2 }));
  ...
  it("warns independently for triangles over budget", () => {
    const listener = vi.fn();
    const unsub = appEvents.on("gpu:budget-warning", listener);
    checkGpuBudget(createPerformanceSnapshot({ fps: 60, drawCalls: 10, triangles: 60_000, geometries: 5, gpuTextures: 2 }));
