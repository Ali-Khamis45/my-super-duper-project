...
describe("createPerformanceSnapshot", () => {
  it("derives frameTimeMs from fps", () => {
    const snapshot = createPerformanceSnapshot({ fps: 60, drawCalls: 10, triangles: 1000, geometries: 5, gpuTextures: 3 });
    expect(snapshot.frameTimeMs).toBeCloseTo(16.667, 2);
  });

  it("handles a zero-fps reading without dividing by zero", () => {
    const snapshot = createPerformanceSnapshot({ fps: 0, drawCalls: 0, triangles: 0, geometries: 0, gpuTextures: 0 });
...
describe("recordPerformanceSnapshot", () => {
  it("is the sole writer of latestSnapshot — a recorded snapshot is immediately readable back", () => {
    const recorded = recordPerformanceSnapshot({ fps: 45, drawCalls: 20, triangles: 2000, geometries: 8, gpuTextures: 4 });
  ...
  it("each call produces a new, distinct snapshot object, never mutating the previous one", () => {
    const first = recordPerformanceSnapshot({ fps: 60, drawCalls: 1, triangles: 1, geometries: 1, gpuTextures: 1 });
    const second = recordPerformanceSnapshot({ fps: 30, drawCalls: 2, triangles: 2, geometries: 2, gpuTextures: 2 });
