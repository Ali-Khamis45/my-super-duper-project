...
| `POST` | `/api/v1/orders/{id}/cancel` | `CancelOrderCommand` | Authenticated, same ownership-or-staff check as above — **implemented, Sprint 5.3, additive**; Sprint 5.5 close-out added a guard rejecting (409 `PaymentCapturedException`) cancellation of an order whose payment is already `Succeeded`/`PartiallyRefunded` — a deliberate hard block, not an automatic refund, since no gateway-level refund call exists yet in this path either |
...
| `POST` | `/api/v1/payments/{id}/confirm` \| `/cancel` | `ConfirmPaymentCommand` \| `CancelPaymentCommand` | Anonymous, ownership-or-staff-or-guest-order check inside the handler (same pattern as Orders), `PaymentPolicy` rate-limited — **implemented, Sprint 5.5**; `cancel` is a real, known limitation — it changes local `Payment` state only, with no `IPaymentGateway` cancel/void call (the interface has none), mitigated by `PaymentCapturedException` blocking cancellation of an already-`Succeeded`/`PartiallyRefunded` payment and by `ProcessPaymentWebhookCommand` logging an error (not a silent no-op) if a late `succeeded` webhook arrives for a payment already cancelled locally — see this sprint's own review for the full disclosure; not fixed this sprint by explicit decision |
