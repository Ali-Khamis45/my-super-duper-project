...
**Compliance record, Sprint 5.3**: zero Sprint 5.1/5.2 contracts touched — every Ordering endpoint/DTO/query-param is new surface. Real usage of the extension mechanisms above: the entire `/api/v1/orders`/`/api/v1/admin/orders` surface (mechanism 2, new endpoints); `CreateOrderRequest.IdempotencyKey` (mechanism 1, an additive required field on a *brand-new* endpoint — not a breaking addition to an existing one, since `POST /api/v1/orders` shipped for the first time this same sprint; added mid-sprint once this sprint's own adversarial review found a real live double-submission bug); `ICurrentUserService.Permissions` (an internal C# interface, not a wire contract this policy governs, but noted here for the same honesty this record already practices — additive, implemented by the one real production class, `HttpCurrentUserService`). No mechanism 5 (new API version) was needed this sprint.

**Compliance record, Sprint 5.4**: zero Sprint 5.1/5.2/5.3 contracts touched — every `/api/v1/admin/inventory/*` endpoint/DTO/query-param is new surface. Real usage of the extension mechanisms above: the entire `/api/v1/admin/inventory/*` surface (mechanism 2, new endpoints — see [31_COMMERCE_ENGINEERING_CONTRACTS.md](31_COMMERCE_ENGINEERING_CONTRACTS.md)'s endpoint catalog for the full list); `GlobalExceptionHandler`'s `Map` switch expression (mechanism 3, additive `case` arms for the eight new `Coffeshop.Domain.Inventory.Exceptions` types — every existing arm, including `DbUpdateConcurrencyException`'s, is untouched); `CoffeshopDbContext`'s three new `DbSet`s and `InventorySeeder` (additive, no existing `DbSet`/seeder modified). One narrow, deliberate exception to "never touch existing code," not a contract change: `DesignTimeDbContextFactory`'s hardcoded connection string gained an explicit `Port=5540` — a real, pre-existing bug (silently connecting `dotnet ef` to a *different* local project's Postgres instance on the implicit default port) found and fixed while generating this sprint's own migration; it changes no runtime behavior and touches no shipped API contract. No mechanism 5 (new API version) was needed this sprint.

**Compliance record, Sprint 5.5**: zero Sprint 5.1/5.2/5.3/5.4 contracts touched — every `/api/v1/payments/*`/`/api/v1/admin/payments/*` endpoint/DTO/query-param is new surface. Real usage of the extension mechanisms above: the entire `/api/v1/payments/*`/`/api/v1/admin/payments/*` surface (mechanism 2, new endpoints); `GatewayPaymentOutcome.RequiresCapture` (mechanism 1, an additive field with a `false` default added mid-sprint once the two-phase capture flow turned out to need a real signal distinct from `Succeeded`, so every existing automatic-capture call site is unaffected); `GlobalExceptionHandler`'s `Map` switch expression (mechanism 3, additive `case` arms for the new `Coffeshop.Domain.Payments.Exceptions` types — every existing arm untouched); `CoffeshopDbContext`'s two new `DbSet`s (additive, no existing `DbSet` modified).

One real, narrow exception, disclosed rather than glossed over: `PayOrderCommand` (an existing Sprint 5.3 endpoint, `POST /api/v1/orders/{id}/pay`) gained a genuine new failure mode — it now throws `PaymentInProgressException` (409) when a real gateway `Payment` is already `Pending`/`Processing` for the order, closing a real double-charge risk this sprint's own adversarial review found (see docs/36_SECURITY_MODEL.md's own Payments security section). By this document's own test, this *is* a behavior change for an existing, unmodified consumer in one specific, narrow scenario (a card checkout genuinely in flight at the same moment staff records cash) — the honest call is that this is a justified security/money-safety fix, not a capability regression: every call that would have succeeded under any *other* circumstance still succeeds identically; only the scenario that used to silently risk a double charge now fails loudly and correctly instead. Treated the same as Sprint 5.4's `DesignTimeDbContextFactory` port fix — a real bug closed, not a contract redesigned.

A second, identical-in-kind exception from this same close-out: `CancelOrderCommand` (also an existing Sprint 5.3 endpoint, `POST /api/v1/orders/{id}/cancel`) gained its own new failure mode — it now throws `PaymentCapturedException` (409) when the order's payment is already `Succeeded`/`PartiallyRefunded`, closing the real strand-a-captured-charge gap the Sprint 5.5 close-out's own review explicitly decided to guard rather than silently auto-refund. Same honest test applied: a behavior change for one specific, narrow, previously-silently-unsafe scenario (cancelling an order whose card charge already went through), not a capability regression — every cancellation that was ever actually safe still succeeds identically. No mechanism 5 (new API version) was needed for either exception this sprint.
