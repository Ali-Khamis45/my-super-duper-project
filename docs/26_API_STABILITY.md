...
| Customizer store, cart store | Future | Fully specified in [18_ENGINEERING_CONTRACTS.md](18_ENGINEERING_CONTRACTS.md) for continuity; Milestone 4 and 8 scope respectively | Milestone 4 / Milestone 8 |
...
| AI Barista data-fetching convention (query keys, loading/error ownership) | Experimental — genuinely undesigned, not just unscheduled | [08_MILESTONES.md](08_MILESTONES.md) explicitly names this as a gap requiring its own short design pass, distinct from every other "Future" row above which already has a settled shape | Milestone 7 (design pass first) |
