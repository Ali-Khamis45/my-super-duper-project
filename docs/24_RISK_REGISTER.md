...
| R-12 | Coffee/foam's spring-damper "cosmetic wobble" (deliberately not a real physics engine) reads as unconvincing once actually built | Medium — a creative-quality risk, not a technical one | Medium — needs a CDR-driven iteration, not a rebuild | Expectations set correctly from the start ("cosmetic wobble, not fluid dynamics" — [04_MOTION_ENGINE.md](04_MOTION_ENGINE.md)); the CDR gate ([09_CREATIVE_DIRECTOR_REVIEW.md](09_CREATIVE_DIRECTOR_REVIEW.md)) exists specifically to catch and iterate on exactly this before shipping | Creative Director / Motion Engineer | **Built, Sprint 3.4** — the wobble is real now, not hypothetical; this sprint's own review includes a Creative Director Review pass against it (bounded ~5° tilt, "not exaggerated" per the brief). Status stays Accepted, not Closed — a CDR score below the ≥9.5 gate on a future full-milestone review would reopen this, per the process the mitigation always relied on |
